...
* Това става чрез извикването на GenServer.cast(pid|name, request), която винаги връща :ok и не
чака за отговор.
...
* Може да се ползва за debugging. <!-- .element: class="fragment" -->
...
#HSLIDE
2. Task.await/2, която взима задача и опционално timeout (по подразбиране 5000, т.е. 5 секунди) и чака резултат от задачата.
...
* ref - Идва от монитора прикрепен към задачата. <!-- .element: class="fragment" -->
...
* Ако пък задачата 'умре', означава, че нещо не е наред. <!-- .element: class="fragment" -->
